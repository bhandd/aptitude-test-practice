...
def find_highlighted_codes(bgr, cx, cy):
    ...
    Returnerar lista med antal (0, 1 eller 2).
    """
    # Kolla om det finns teal-highlight i zonen ovanför röret
    y1 = max(0, cy - 200)
    y2 = cy - 50
    x1 = max(0, cx - 300)
    x2 = cx + 300
    ...
    # Hitta separata highlight-regioner
    kernel = np.ones((10,10), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    significant = [c for c in contours if cv2.contourArea(c) > 2000]
...
def analyze_and_solve(bgr):
    ...
    if n_highlighted > 0:
        ...
        for i in range(n_highlighted):
            # Highlightade koder sitter ovanför tratten
            # Mätt från debug: permkod sitter ca 70-130px ovanför cy
            slot_h = 70
            gap_from_cy = 70 + slot_h * (n_highlighted - 1 - i)
            hl_y1 = cy - gap_from_cy - slot_h
            hl_y2b = cy - gap_from_cy
            hl_zone = bgr[max(0,hl_y1):max(0,hl_y2b), max(0,cx-180):cx+180]
            if DEBUG:
